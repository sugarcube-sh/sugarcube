...
import { getKeysForPath, tokenPathsMatching } from "./TokenStore";

/**
 * Generic scale cascade: base + spread transforms over a group of
 * dimension tokens.
 *
 * The tweakpane exposes two knobs per scale binding: a BASE size and a
 ...
 */
export function captureScale(pathPattern: string, basePath: string): CapturedScale | null {
    const baseKeys = getKeysForPath(basePath);
    const baseToken = baseKeys[0] ? snapshot.resolved[baseKeys[0]] : undefined;
    const baseFluid = baseToken ? readFluidValues(baseToken) : null;
    if (!baseFluid || baseFluid.max === 0 || baseFluid.min === 0) return null;

    const paths = tokenPathsMatching(pathPattern);
    const steps: CapturedStep[] = [];

    for (const path of paths) {
        const keys = getKeysForPath(path);
        const token = keys[0] ? snapshot.resolved[keys[0]] : undefined;
...
export function applyScaleToResolved(
    ...
    for (const step of scale.steps) {
        ...
        const keys = getKeysForPath(step.path);
 ...
 * control mount time.
 */
export function captureLinkedScale(pathPattern: string): CapturedLinkedScale {
