...
The base directory for generated CSS files. Path is relative to your project root. Other CSS output paths (`variables`, `utilities`, `cube`) use this as their default base, but can be overridden to point anywhere.
...
The directory where CSS variable files (from design tokens) are written. Path is relative to your project root. If not specified, defaults to the `global` subdirectory under `cssRoot`.
...
export default defineConfig({
  ...
  output: {
    cssRoot: "src/styles",
    variables: "src/tokens"  // Can point anywhere relative to project root
  }
});
```
...
The directory where utility class CSS files are written. Path is relative to your project root. If not specified, defaults to the `utilities` subdirectory under `cssRoot`.
...
export default defineConfig({
  ...
  output: {
    cssRoot: "src/styles",
    utilities: "public/css/utils"  // Can point anywhere relative to project root
  }
});
```
...
The directory where CUBE CSS files are written when using the `cube` command. Path is relative to your project root. If not specified, defaults to the same directory as `cssRoot`.
...
Wraps generated CSS in `@layer` blocks for CSS Cascade Layers support.
...
- **`variables`** (required): The CSS layer name for token variables
- **`utilities`** (required): The CSS layer name for utility classes
...
These defaults are applied automatically when you have a config file. When running without a config, paths are auto-detected based on whether your project has a `src/` directory (e.g. `cube` and `components` use `src/styles` or `styles`, and `src/components/ui` or `components/ui`). You only need to specify values when you want to override them.
