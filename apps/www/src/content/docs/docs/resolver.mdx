...
- **`$extensions.sh.sugarcube.selector`** — CSS selector pattern with `{context}` placeholder

<SiteAside type="note">
  Without a `selector` or `atRule` extension, only the default context outputs to `:root`. Add an extension to enable multi-context output.
</SiteAside>
