...
@import "./variables.gen.css" layer(base);
@import "./utilities.gen.css" layer(utilities);
```
...
If you prefer the layer wrapper baked into the generated files, use the `layer` option:
...
```css title="tokens.css" frame="none"
@layer base {
    :root { --color-primary: ...; }
}
```
...
@import "./variables.gen.css";
@import "./utilities.gen.css";
```
