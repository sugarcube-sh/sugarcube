...
If you want themes to follow the user's OS preference without JavaScript, you can use `prefers-color-scheme` media queries instead of data attributes.

Add the `selector` option to your modifier's `$extensions`:
...
<SiteAside type="note">
  The `prefers-color-scheme` selector only supports `light` and `dark` contexts. If your modifier has other contexts (like `high-contrast`), use the default data-attribute approach instead.
</SiteAside>

### When to use each approach

**Use data attributes** when you need a theme toggle UI. Your JavaScript reads `prefers-color-scheme` and sets the data attribute, giving users control to override their OS preference.

**Use `prefers-color-scheme`** when you don't need a toggle — the theme follows the OS automatically with zero JavaScript.
