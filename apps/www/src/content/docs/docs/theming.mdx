...
Add `prefersColorScheme: true` to your modifier's `$extensions`:
...
<SiteAside type="note">
  Context names must be `light` and `dark` — these map directly to `prefers-color-scheme: light` and `prefers-color-scheme: dark` in CSS. If your modifier has other contexts (like `high-contrast`), use the default data-attribute approach instead.
</SiteAside>

### When to use each approach

**Use data attributes** when users should be able to override their OS preference via a theme toggle.
