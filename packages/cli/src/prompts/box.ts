...
export const box = (message = "", title = "", opts?: BoxOptions) => {
    ...
    if (opts?.width === "auto") {
        ...
        let longestLine = strip(title).length + titlePadding * 2;
        for (const line of lines) {
            const lineWithPadding = strip(line).length + contentPadding * 2;
    ...
    const truncatedTitle =
        strip(title).length > maxTitleLength ? `${title.slice(0, maxTitleLength - 3)}...` : title;
