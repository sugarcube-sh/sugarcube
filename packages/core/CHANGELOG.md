...
- d500b1a: Fix a few DTCG conformance issues. Font weight names are now case-sensitive. A gradient stop position below 0 or above 1 is no longer an error. A transition's delay is properly marked 'required' in the types (brining it into line with what the validator already enforced).

## 0.2.18

### Patch Changes

- 8b7b78e: fix: omit redundant `@media` blocks when a breakpoint resolves to the same value as the one before it. Thanks @aarongeorge for raising this!
