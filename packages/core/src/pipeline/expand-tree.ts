...
import { mergeGroups } from "../utils/merge-groups.js";

export type ExpandTreeError = {
    path: string;
    message: string;
    sourcePath: string;
};

export type ExpandTreeResult = {
    trees: TokenTree[];
    errors: ExpandTreeError[];
};
...
type EntryResult =
    | { value: Token | TokenGroup; errors?: ExpandTreeError[] }
    | { error: ExpandTreeError };
...
type ExpandSingleResult =
    ...
    | { result?: undefined; error: ExpandTreeError };
 ...
 */
export function expandTree(trees: TokenTree[]): ExpandTreeResult {
    const results: TokenTree[] = [];
    const errors: ExpandTreeError[] = [];
